...
def test_social_footprint(client):
    response = client.post('/api/social_footprint', json={"platform": "twitter", "username": "testuser"})
...
def test_commercial_api(client):
    response = client.post('/api/commercial', json={"query": "test"}, headers={"x-api-key": "demo-key"})
    assert response.status_code == 200
    data = response.get_json()
    assert 'result' in data
